...
var isValidSudoku = function(board) {
  let row = new Array(9).fill(0).map(i => new Array(9).fill(0))
  let col = new Array(9).fill(0).map(i => new Array(9).fill(0))
  let subPiece = new Array(3).fill(0).map(i => new Array(3).fill(0).map(j => new Array(9).fill(0)))
  for(let i = 0; i < 9; i++) {
    for(let j = 0; j < 9; j++) {
      let char = board[i][j]
      if (char !== '.') {
        ...
        row[i][idx]++
        col[j][idx]++
        subPiece[Math.floor(i / 3)][Math.floor(j / 3)][idx]++
        if (row[i][idx] > 1 || col[j][idx] > 1 || subPiece[Math.floor(i / 3)][Math.floor(j / 3)][idx] > 1) {
          return false
        }
      }
    }
